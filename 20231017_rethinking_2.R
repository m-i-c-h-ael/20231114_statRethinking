...
rnd= seq(0.1,10,.1)
prods= sapply(rep(10,10000), function(x) {prod(sample(rnd,x,replace=TRUE))})
plot(density(prods),xlim= c(0,10000000))

log_mean= mean(log(prods))
log_sd= sd(log(prods))

curve(dlnorm(x,meanlog=log_mean,sdlog=log_sd),add=TRUE,col='red')
...
cher= cher[ apply(cher,1, function(x){!any(is.na(x))}),]
...
B= bs(x=cher$year, knots= knots, degree=1,intercept=TRUE)
for(i in 1:dim(B)[2]){
...
x= 23:30; y=0:35
z_pred= outer(X=x,Y=y,FUN=function(x,y){coef(mod3D)[1] + coef(mod3D)[2]*x + coef(mod3D)[3]*y})

#######
coef_x=3
coef_y=0
x=seq(0,10,.5); y= seq(0,10,.5)
#df= expand.grid(x=x,y=y)
z= outer(X=x, Y=y, FUN= function(x,y){coef_x*x + coef_y*y})
...
mN_priorPred= with(mN_priorSamp, sapply(1:length(a), function(i) a[i] + bN[i]*milk$nec))
...
nec_vec= seq(-2,2,.1)
mN_postPred= with(mN_postSamp, sapply(1:length(a), function(i) a[i] + bN[i]*nec_vec))
...
dv= WaffleDivorce
head(dv)
scatter3D_fancy(x=dv$MedianAgeMarriage,y=dv$Marriage,z=dv$Divorce, bty='b2',theta=30,phi=30,
...
scatter3D_fancy(x=dv$MedianAgeMarriage,y=dv$Marriage,z=dv$Divorce, bty='b2',theta=30,phi=30,
                xlim=c(20,30),ylim=c(13,30))
scatter3D_fancy(x=AMgrid$A,y=AMgrid$M,z=AMgrid$D_lmA, bty='b2',theta=30,phi=30,add=TRUE,
...
scatter3D_fancy(x=dv$MedianAgeMarriage,y=dv$Marriage,z=dv$Divorce, bty='b2',theta=30,phi=30,
                xlim=c(20,30),ylim=c(13,30))
scatter3D_fancy(x=AMgrid$A,y=AMgrid$M,z=AMgrid$D_lmM, bty='b2',theta=30,phi=30,add=TRUE,
...
AMgrid$D_lmAM= coef(lmAM)[1] + coef(lmAM)[2]*AMgrid$A + coef(lmAM)[3]*AMgrid$M
scatter3D_fancy(x=dv$MedianAgeMarriage,y=dv$Marriage,z=dv$Divorce, bty='b2',theta=30,phi=30,
                xlim=c(20,30),ylim=c(13,30))
scatter3D_fancy(x=AMgrid$A,y=AMgrid$M,z=AMgrid$D_lmAM, bty='b2',theta=30,phi=30,add=TRUE,
...
scatter3D_fancy(x=AMgrid$A,y=AMgrid$M,z=AMgrid$D_lmAM, bty='b2',theta=30,phi=30,
                xlim=c(20,30),ylim=c(13,30))
scatter3D_fancy(x=AMgrid$A,y=AMgrid$M,z=AMgrid$D_lmAM, bty='b2',theta=30,phi=30,add=TRUE,
                mainCol='orange',type='l',XZwallCol=NA,YZwallCol=NA)
...
AMgrid$D_causA= coef(lmAM)[1] + coef(lmAM)[2]*AMgrid$A + coef(lmAM)[3]*AMgrid$MfromA
scatter3D_fancy(x=AMgrid$A,y=AMgrid$MfromA,z=AMgrid$D_causA, bty='b2',theta=30,phi=30,
                xlim=c(20,30),ylim=c(13,30))
scatter3D_fancy(x=AMgrid$A,y=AMgrid$M,z=AMgrid$D_lmAM, bty='b2',theta=30,phi=30,add=TRUE,
                mainCol='orange',type='l',XZwallCol=NA,YZwallCol=NA)
...
F= rbinom(n=N, size=1, prob= 0.5-0.4*T+0.4*M)
...
# fit models
trainFit20_deg2= quap( alist( y ~ dnorm(mu, sigma),
             mu <- a1*x + a2*x^2,
             a1 ~ dnorm(0, 0.5),
             a2 ~ dnorm(0, 0.5),
             sigma ~ dexp(1)
...
trainFit20_deg3= quap( alist( y ~ dnorm(mu, sigma),
...
trainFit20_deg4= quap( alist( y ~ dnorm(mu, sigma),
                              mu <- a1*x + a2*x^2 + a3*x^3 + a4*x^4,
                              a1 ~ dnorm(0, 0.5),
                              a2 ~ dnorm(0, 0.5),
                              a3 ~ dnorm(0, 0.5),
                              a4 ~ dnorm(0, 0.5),
...
lppi=  sapply(1:ncases, function(i){log_sum_exp(logprob[i,]) - log(1000)})
pWAIC= sapply(1:ncases, function(i){var(logprob[i,])})
